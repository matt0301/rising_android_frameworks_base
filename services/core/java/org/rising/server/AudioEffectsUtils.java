...
public class AudioEffectsUtils {
    ...
    public void initializeEffects() {
        ...
        int audioSessionId = getAudioSession();
        mEqualizer = new Equalizer(EFFECT_PRIORITY, audioSessionId);
        mBassBoost = new BassBoost(EFFECT_PRIORITY, audioSessionId);
        mPresetReverb = new PresetReverb(EFFECT_PRIORITY, audioSessionId);
        mVirtualizer = new Virtualizer(EFFECT_PRIORITY, audioSessionId);
    ...
    public int getAudioSession() {
        int mAudioSessionId = 0; // global output mix
        List<AudioRecordingConfiguration> configs = mAudioManager.getActiveRecordingConfigurations();
        if (configs != null && !configs.isEmpty()) {
            mAudioSessionId = configs.get(0).getClientAudioSessionId() - 8;
        }
        ...
        if (!getEffectEnabled(effect)) {
            if (effect == null) {
                if (effect instanceof BassBoost) {
                    effect = new BassBoost(EFFECT_PRIORITY, getAudioSession());
                } else if (effect instanceof Equalizer) {
                    effect = new Equalizer(EFFECT_PRIORITY, getAudioSession());
                } else if (effect instanceof LoudnessEnhancer) {
                    effect = new LoudnessEnhancer(EFFECT_PRIORITY, getAudioSession());
                } else if (effect instanceof PresetReverb) {
                    effect = new PresetReverb(EFFECT_PRIORITY, getAudioSession());
                } else if (effect instanceof Virtualizer) {
                    effect = new Virtualizer(EFFECT_PRIORITY, getAudioSession());
                }
            }
